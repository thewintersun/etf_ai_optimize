...
PARAMS = {
    ...
    # 信号去极值 + 压缩（1.1）
    "signal_clip_quantile": 0.0,  # 分位数裁剪强度：0.1 表示按 10%/90% 分位截断
    "signal_tanh_scale": 1.0,  # tanh 压缩尺度：越大越“线性”，越小压缩越强

    # 信号融合（1.2）：alpha=1 只用横截面(mean/std)；alpha=0 只用时间序列(ts_sharpe)
    "signal_blend_alpha": 0.0,

    # 调仓过滤/平滑（方向2）
    "rebalance_min_delta": 0.40,  # 若 sum(|new-old|) < 阈值，则跳过本次调仓（0 表示不启用）
    "rebalance_smoothing_beta": 1.0,  # 权重平滑系数 beta：1=不平滑；0.5=半仓渐进到目标
